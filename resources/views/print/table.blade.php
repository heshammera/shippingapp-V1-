...
@php
    ...
    $totalPieces = $shipments->flatMap->products->sum(fn($p) => $p->pivot->quantity);
@endphp
...
<div class="d-flex justify-content-between align-items-start" style="width: 100%;">
    ...
    <div class="text-end">
        ...
        <h3 class="Slogan">{{ \App\Models\Setting::getValue('company_name', 'اسم المتجر') }}</h3>
            ...
            @foreach($shipments as $shipment)
                <tr class="{{ $shipment->status?->row_color }}">
                    <td>
                        ...
                        @endphp
                        <img src="{{ $barcodeSrc }}" class="barcode" style="width: 80px; height: 30px;"><br>
...
<td style="font-size: 14px; text-align: center;">
    {{ $shipment->products->sum(fn($p) => $p->pivot->quantity) }}
</td>
